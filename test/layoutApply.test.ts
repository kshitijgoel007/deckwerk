...
describe('Apply layout button', () => {
  ...
  function panelFor(deck: Deck) {
    ...
    return { store, panel, button, setStatusMessage, save, onPreviewSlide };
  }
  ...
  it('is a heading and one button whose label follows the theme scope', () => {
    ...
    // A one-line readout of what the button would do sits above it.
    expect(section.querySelector('.layout-readout')!.textContent).toBe('2 boxes re-aligned');
  ...
  it('says so, and disables the button, when the scope has nothing to align', () => {
    const deck = nudgedDeck();
    const { store, panel, button } = panelFor(deck);
    store.selectSlide(2);
    panel.syncScope(1);
    expect(button.disabled).toBe(true);
    expect(panel.element.querySelector('.layout-readout')!.textContent)
      .toBe('Freeform slides have no layout to align to.');
  });

  it('previews the re-aligned current slide on the canvas while the button is hovered', () => {
    const deck = nudgedDeck();
    const { store, panel, button, onPreviewSlide } = panelFor(deck);
    store.selectSlide(1);
    panel.syncScope(1);
    button.dispatchEvent(new Event('mouseenter'));
    const [shown, label] = onPreviewSlide.mock.calls.at(-1)!;
    expect(label).toBe('Apply layout');
    expect(shown.id).toBe('s2');
    expect(shown.elements.find((el: TextEl) => el.layoutPlaceholder === 'title').x).toBe(120);
    // Nothing committed: the real slide is still nudged.
    expect(slot(store.get().deck, 1, 'title').x).toBe(400);
