  ...
  it('patches broad UI properties in one lossless operation batch', () => {
    ...
    const result = applyNativeEdits(before, [
      ...
      },
      {
        ...
        set: { name: 'Unified', 'background.color': '#101218', layout: 'standard', magicMoveFromPrevious: true, skipped: false },
        unset: [],
      },
      {
        target: 'deck',
        set: { title: 'Unified deck', magicMoveDuration: 850, magicMoveEasing: 'ease-out', 'themeStyle.fonts.title.family': 'Inter' },
        unset: [],
      },
    ]);
    ...
    expect(slide).toMatchObject({ name: 'Unified', background: { color: '#101218', image: null }, layout: 'standard', magicMoveFromPrevious: true, skipped: false });
    expect(result.deck).toMatchObject({ title: 'Unified deck', magicMoveDuration: 850, magicMoveEasing: 'ease-out' });
