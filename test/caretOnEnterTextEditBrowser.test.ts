...
/**
 * Where the caret lands when a click opens a text edit.
 *
 * Reported: double-clicking a text box puts the caret at the *start* of the
 * box rather than under the pointer. The canvas suppresses the browser's own
 * selection on pointerdown (it would sweep-select across slide objects), so
 * the caret has to be reconstructed from the click point — and every way that
 * reconstruction can fail parks the freshly focused contenteditable at offset
 * 0, which is the whole box away from where the author clicked.
 *
 * The gestures here are the real ones: CDP mouse input over measured glyph
 * boxes, through the shipped desktop shell.
 */
...
describe.skipIf(!electronBinary)('caret placement when a click opens a text edit', () => {
  it('takes the clicked word, and never falls back to the start of the box', {
    ...
    desktop = await launchDesktopEditor(TEXT_ID, HTML);
